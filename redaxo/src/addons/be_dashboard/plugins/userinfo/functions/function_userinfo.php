...
function rex_a659_statistics()
{
    ...
    $sql = rex_sql::factory();
//  $sql->setDebug();
    $result = $sql->getArray('SELECT COUNT(*) as count, updatedate FROM ' . rex::getTablePrefix() . 'article WHERE clang=0 AND startarticle=1 GROUP BY clang ORDER BY updatedate DESC');
    if (count($result) > 0) {
