...
function rex_a659_statistics()
{
    ...
    $sql = rex_sql::factory();
//  $sql->setDebug();
    $result = $sql->getArray('SELECT COUNT(*) as count, updatedate FROM ' . rex::getTablePrefix() . 'article WHERE clang_id=1 AND startarticle=1 GROUP BY clang_id ORDER BY updatedate DESC');
    if (count($result) > 0) {
    ...
    $result = $sql->getArray('SELECT COUNT(*) as count, updatedate FROM ' . rex::getTablePrefix() . 'article WHERE clang_id=1 GROUP BY clang_id ORDER BY updatedate DESC');
    if (count($result) > 0) {
    ...
    $result = $sql->getArray('SELECT COUNT(*) as count FROM ' . rex::getTablePrefix() . 'clang_id');
    if (count($result) > 0) {
