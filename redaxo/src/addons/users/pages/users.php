...
if ($SHOW) {
    ...
    $thIcon = '<a class="rex-link" href="' . $list->getUrl(['FUNC_ADD' => '1']) . '"' . rex::getAccesskey(rex_i18n::msg('create_user'), 'add') . ' title="' . rex_i18n::msg('create_user') . '"><i class="rex-icon rex-icon-add-user"></i></a>';
    ...
    if (rex::getUser()->isAdmin()) {
        ...
        $list->setColumnFormat('impersonate', 'custom', static function ($params) use ($list) {
            ...
            return sprintf('<a class="rex-link" href="%s" data-pjax="false"><i class="rex-icon rex-icon-sign-in"></i> %s</a>', $url, rex_i18n::msg('login_impersonate'));
        });
    }
