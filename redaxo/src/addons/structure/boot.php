...
$myaddon->setProperty('start_article_id', $myaddon->getConfig('start_article_id', 1));
$myaddon->setProperty('notfound_article_id', $myaddon->getConfig('notfound_article_id', 1));

if (rex_request('article_id', 'int') == 0) {
    $myaddon->setProperty('article_id', rex_article::getSiteStartArticleId());
} else {
    $article_id = rex_request('article_id', 'int');
    $article_id = rex_article::get($article_id) ? $article_id : rex_article::getNotfoundArticleId();
    $myaddon->setProperty('article_id', $article_id);
}

if (rex::isBackend() && rex::getUser()) {
    rex_view::addJsFile($myaddon->getAssetsUrl('linkmap.js'));
