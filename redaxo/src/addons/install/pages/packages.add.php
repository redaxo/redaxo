...
if ($addonkey && isset($addons[$addonkey]) && !rex_addon::exists($addonkey)) {
    ...
    $fragment->setVar('title', '<b>' . $addonkey . '</b> ' . $addon->i18n('information'), false);
    ...
    $content = '
        <table class="table table-striped table-hover">
            ...
            <tr>
                ...
                <th class="rex-table-width-3">' . $addon->i18n('version') . '</th>
                <th class="rex-table-width-3"><span class="text-nowrap">' . $addon->i18n('published_on') . '</span></th>
                <th>' . $addon->i18n('description') . '</th>
                <th class="rex-table-action">' . $addon->i18n('header_function') . '</th>
            </tr>
            </thead>
            <tbody>';
    ...
    foreach ($addon['files'] as $fileId => $file) {
        ...
        $content .= '
            <tr>
                ...
                <td data-title="' . $addon->i18n('published_on') . '">' . rex_escape(rex_formatter::strftime($file['created'])) . '</td>
                <td data-title="' . $addon->i18n('description') . '">' . nl2br($file['description']) . '</td>
                <td class="rex-table-action"><a href="' . rex_url::currentBackendPage(['addonkey' => $addonkey, 'file' => $fileId] + rex_api_install_package_add::getUrlParams()) . '" data-pjax="false"><i class="rex-icon rex-icon-download"></i> ' . $addon->i18n('download') . '</a></td>
            </tr>';
    }
    ...
    $toolbar = '
        <div class="form-group form-group-xs">
            <div class="input-group input-group-xs" id="rex-js-install-addon-search">
                <input class="form-control" type="text" autofocus placeholder="' . $addon->i18n('search') . '" />
                <span class="input-group-btn"><button class="btn btn-default">' . $addon->i18n('clear') . '</button></span>
            </div>
        </div>
    ';
        ...
        <table class="table table-striped table-hover" id="rex-js-table-install-packages-addons">
         <thead>
            <tr>
                ...
                <th><a href="'.rex_url::currentBackendPage().'" title="'.$addon->i18n('sort_default').'">' . $addon->i18n('key') . '</a></th>
                <th>' . $addon->i18n('name') . ' / ' . $addon->i18n('author') . '</th>
                <th class="rex-table-min-width-3"><a href="'.rex_url::currentBackendPage(['sort' => $sortNext]).'" title="' . $addon->i18n('sort') . '"><span class="text-nowrap">' . $addon->i18n('published_on') . '</span>&nbsp;<span><i class="rex-icon rex-icon-sort fa-sort'.$sortClass.'"></i></span></a></th>
                <th>' . $addon->i18n('shortdescription') . '</th>
                <th class="rex-table-action">' . $addon->i18n('header_function') . '</th>
            </tr>
         </thead>
    ...
    foreach ($addons as $key => $addon) {
        if (rex_addon::exists($key)) {
            $content .= '
                <tr>
                    <td class="rex-table-icon"><i class="rex-icon rex-icon-package"></i></td>
                    <td data-title="' . $addon->i18n('key') . '">' . $key . '</td>
                    <td data-title="' . $addon->i18n('name') . '"><b>' . $addon['name'] . '</b><br /><span class="text-muted">' . rex_escape($addon['author']) . '</span></td>
                    <td data-title="' . $addon->i18n('published_on') . '">' . rex_escape(rex_formatter::strftime(reset($addon['files'])['created'])) . '</td>
                    <td data-title="' . $addon->i18n('shortdescription') . '">' . nl2br($addon['shortdescription']) . '</td>
                    <td class="rex-table-action"><span class="text-nowrap"><i class="rex-icon rex-icon-package-exists"></i> ' . $addon->i18n('addon_already_exists') . '</span></td>
                </tr>';
        } else {
            ...
            $content .= '
                <tr>
                    ...
                    <td data-title="' . $addon->i18n('key') . '"><a href="' . $url . '">' . rex_escape($key) . '</a></td>
                    <td data-title="' . $addon->i18n('name') . '"><b>' . rex_escape($addon['name']) . '</b><br /><span class="text-muted">' . $addon['author'] . '</span></td>
                    <td data-title="' . $addon->i18n('published_on') . '">' . rex_escape(rex_formatter::strftime(reset($addon['files'])['created'])) . '</td>
                    <td data-title="' . $addon->i18n('shortdescription') . '">' . nl2br(rex_escape($addon['shortdescription'])) . '</td>
    ...
    $fragment = new rex_fragment();
    $fragment->setVar('title', $addon->i18n('addons_found', count($addons)), false);
