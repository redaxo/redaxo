...
if ($addonkey && isset($addons[$addonkey])) {
    ...
    if ($file_id) {
        ...
        $n = [];
        $n['label'] = '<label for="rex-js-install-packages-upload-version">' . $myaddon->i18n('version') . '</label>';
        ...
        $n = [];
        $n['label'] = '<label for="rex-install-packages-upload-description">' . $myaddon->i18n('description') . '</label>';
        ...
        $n = [];
        $n['reverse'] = true;
        $n['label'] = '<label for="rex-install-packages-upload-status">' . $myaddon->i18n('online') . '</label>';
        ...
        $n = [];
        $n['reverse'] = true;
        $n['label'] = '<label for="rex-js-install-packages-upload-upload-file">' . $myaddon->i18n('upload_file') . '</label>' . $hiddenField;
        ...
        if (rex_addon::get($addonkey)->isInstalled() && is_dir(rex_url::addonAssets($addonkey))) {
            $n = [];
            $n['reverse'] = true;
            $n['label'] = '<label for="rex-js-install-packages-upload-replace-assets">' . $myaddon->i18n('replace_assets') . '</label>';
        ...
        if (is_dir(rex_path::addon($addonkey, 'tests'))) {
            $n = [];
            $n['reverse'] = true;
            $n['label'] = '<label for="rex-js-install-packages-upload-ignore-tests">' . $myaddon->i18n('ignore_tests') . '</label>';
        ...
        $n = [];
        $n['field'] = '<button class="btn btn-save rex-form-aligned" type="submit" name="upload[send]" value="' . $myaddon->i18n('send') . '">' . $myaddon->i18n('send') . '</button>';
        $formElements[] = $n;

        $n = [];
        $n['field'] = '<button class="btn btn-delete" value="' . $myaddon->i18n('delete') . '" onclick="if(confirm(\'' . $myaddon->i18n('delete') . ' ?\')) location.href=\'' . rex_url::currentBackendPage(['addonkey' => $addonkey, 'file' => $file_id] + rex_api_install_package_delete::getUrlParams()) . '\';">' . $myaddon->i18n('delete') . '</button>';
        $formElements[] = $n;
        ...
        $fragment->setVar('title', $addonkey . ' <small>' . $myaddon->i18n($new ? 'file_add' : 'file_edit') . '</small>', false);
    ...
    //--></script>';
        ...
        if (rex_addon::exists($addonkey)) {
            $icon = '<a href="' . rex_url::currentBackendPage(['addonkey' => $addonkey, 'file' => 'new']) . '" title="' . $myaddon->i18n('file_add') . '"><i class="rex-icon rex-icon-add-package"></i></a>';
        }
        ...
        <table class="table">
            <tbody>
            <tr>
                <th class="rex-table-width-5">' . $myaddon->i18n('name') . '</th>
                <td data-title="' . $myaddon->i18n('name') . '">' . rex_escape($addon['name']) . '</td>
            </tr>
            <tr>
                <th>' . $myaddon->i18n('author') . '</th>
                <td data-title="' . $myaddon->i18n('author') . '">' . rex_escape($addon['author']) . '</td>
            </tr>
            <tr>
                <th>' . $myaddon->i18n('shortdescription') . '</th>
                <td data-title="' . $myaddon->i18n('shortdescription') . '">' . nl2br(rex_escape($addon['shortdescription'])) . '</td>
            </tr>
            <tr>
                <th>' . $myaddon->i18n('description') . '</th>
                <td data-title="' . $myaddon->i18n('description') . '">' . nl2br(rex_escape($addon['description'])) . '</td>
            </tr>
            </tbody>
        </table>';

        $fragment = new rex_fragment();
        $fragment->setVar('title', $addonkey . ' <small>' . $myaddon->i18n('information') . '</small>', false);
        ...
        <table class="table table-striped table-hover">
            ...
            <tr>
                ...
                <th class="rex-table-width-4">' . $myaddon->i18n('version') . '</th>
                <th>REDAXO</th>
                <th>' . $myaddon->i18n('description') . '</th>
                <th class="rex-table-action" colspan="2">' . $myaddon->i18n('status') . '</th>
            </tr>
            </thead>
            <tbody>';
        ...
        foreach ($addon['files'] as $fileId => $file) {
            ...
            <tr>
                ...
                <td data-title="' . $myaddon->i18n('version') . '">' . rex_escape($file['version']) . '</td>
                <td data-title="REDAXO">' . rex_escape(implode(', ', $file['redaxo_versions'])) . '</td>
                <td data-title="' . $myaddon->i18n('description') . '">' . nl2br(rex_escape($file['description'])) . '</td>
                <td class="rex-table-action"><a href="' . $url . '"><i class="rex-icon rex-icon-edit"></i> ' . $myaddon->i18n('file_edit') . '</a></td>
                <td class="rex-table-action"><span class="rex-text-' . $status . '"><i class="rex-icon rex-icon-' . $status . '"></i> ' . $myaddon->i18n($status) . '</span></td>
            </tr>';
        }
        ...
        $fragment = new rex_fragment();
        $fragment->setVar('title', $myaddon->i18n('files'), false);
        $fragment->setVar('content', $panel, false);
        $content = $fragment->parse('core/page/section.php');
    ...
    $panel = '
        <table class="table table-striped table-hover">
         <thead>
            <tr>
                <th class="rex-table-icon"><a href="' . rex_url::currentBackendPage(['func' => 'reload']) . '" title="' . $myaddon->i18n('reload') . '"><i class="rex-icon rex-icon-refresh"></i></a></th>
                <th>' . $myaddon->i18n('key') . '</th>
                <th>' . $myaddon->i18n('name') . '</th>
                <th class="rex-table-action" colspan="2">' . $myaddon->i18n('status') . '</th>
            </tr>
         </thead>
         <tbody>';
    ...
    foreach ($addons as $key => $addon) {
        ...
        $panel .= '
            <tr>
                <td class="rex-table-icon"><a href="' . $url . '"><i class="rex-icon rex-icon-package"></i></a></td>
                <td data-title="' . $myaddon->i18n('key') . '">' . rex_escape($key) . '</td>
                <td data-title="' . $myaddon->i18n('name') . '">' . rex_escape($addon['name']) . '</td>
                <td class="rex-table-action"><a href="' . $url . '"><i class="rex-icon rex-icon-view"></i> ' . rex_i18n::msg('view') . '</a></td>
                <td class="rex-table-action"><span class="rex-text-' . $status . '">' . $myaddon->i18n($status) . '</span></td>
            </tr>';
    }
    ...
    $fragment = new rex_fragment();
    $fragment->setVar('title', $myaddon->i18n('my_packages'), false);
    $fragment->setVar('content', $panel, false);
    $content = $fragment->parse('core/page/section.php');
