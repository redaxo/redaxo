...
$configFile = $myaddon->getDataPath('config.json');
$config = array_merge([
    'backups' => false,
    'api_login' => null,
    'api_key' => null,
...
if (is_array($newConfig)) {
    ...
    if (rex_file::putCache($configFile, $config)) {
        echo rex_view::success($myaddon->i18n('settings_saved'));
        rex_install_webservice::deleteCache();
    } else {
        echo rex_view::error($myaddon->i18n('settings_error', $configFile));
    }
}

$panel .= '
            <fieldset>
                <legend>' . $myaddon->i18n('settings_general') . '</legend>';

            $formElements = [];

                $n = [];
                $n['reverse'] = true;
                $n['label'] = '<label>' . $myaddon->i18n('settings_backups') . '</label>';
...
$panel .= '
            ...
            <fieldset>
                <legend>' . $myaddon->i18n('settings_myredaxo_account') . '</legend>';

            $formElements = [];

                $n = [];
                $n['label'] = '<label for="install-settings-api-login">' . $myaddon->i18n('settings_api_login') . '</label>';
                ...
                $n = [];
                $n['label'] = '<label for="install-settings-api-key">' . $myaddon->i18n('settings_api_key') . '</label>';
...
$fragment->setVar('title', $myaddon->i18n('subpage_settings'), false);
