...
if ($core && !empty($coreVersions)) {
    ...
    foreach ($coreVersions as $id => $version) {
        $panel .= '
                <tr>
                    ...
                    <td data-title="' . $addon->i18n('version') . '">' . rex_escape($version['version']) . '</td>
                    <td data-title="' . $addon->i18n('description') . '">' . nl2br(rex_escape($version['description'])) . '</td>
                    <td class="rex-table-action"><a href="' . rex_url::currentBackendPage(['core' => 1, 'version_id' => $id] + rex_api_install_core_update::getUrlParams()) . '" data-pjax="false">' . $addon->i18n('update') . '</a></td>
                </tr>';
    }
    ...
    foreach ($addon['files'] as $fileId => $file) {
        $panel .= '
            <tr>
                ...
                <td class="rex-table-action"><a href="' . rex_url::currentBackendPage(['addonkey' => $addonkey, 'file' => $fileId] + rex_api_install_package_update::getUrlParams()) . '" data-pjax="false">' . $addon->i18n('update') . '</a></td>
            </tr>';
    }
    ...
    $panel = '
        <table class="table table-striped table-hover">
            ...
            <tr>
                ...
                <th>' . $addon->i18n('existing_version') . '</th>
                <th>' . $addon->i18n('available_versions') . '</th>
            </tr>
            </thead>
            <tbody>';
    ...
    if (!empty($coreVersions)) {
        ...
        $panel .= '
            <tr>
                ...
                <td data-title="' . $addon->i18n('key') . '"><a href="' . $url . '">core</a></td>
                <td data-title="' . $addon->i18n('name') . '">REDAXO Core</td>
                <td data-title="' . $addon->i18n('existing_version') . '">' . rex_escape(rex::getVersion()) . '</td>
                <td data-title="' . $addon->i18n('available_versions') . '">' . rex_escape(implode(', ', $availableVersions)) . '</td>
            </tr>';
    }
    ...
    foreach ($addons as $key => $addon) {
        ...
        $panel .= '
            <tr>
                ...
                <td data-title="' . $addon->i18n('name') . '">' . rex_escape($addon['name']) . '</td>
                <td data-title="' . $addon->i18n('existing_version') . '">' . rex_escape(rex_addon::get($key)->getVersion()) . '</td>
                <td data-title="' . $addon->i18n('available_versions') . '">' . rex_escape(implode(', ', $availableVersions)) . '</td>
            </tr>';
    }
    ...
    $fragment = new rex_fragment();
    $fragment->setVar('title', $addon->i18n('available_updates', !empty($coreVersions) + count($addons)), false);
    $fragment->setVar('content', $panel, false);
    $content = $fragment->parse('core/page/section.php');
}
