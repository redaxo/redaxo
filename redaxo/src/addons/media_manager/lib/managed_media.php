...
{
    const PROP_JPG_QUALITY = 'jpg_quality';
    const PROP_PNG_COMPRESSION = 'png_compression';
    const PROP_WEBP_QUALITY = 'webp_quality';
    const PROP_INTERLACE = 'interlace';
    ...
    protected function getImageSource()
    {
        ...
        $interlace = $this->getImageProperty(rex_managed_media::PROP_INTERLACE, $addon->getConfig('interlace', ['jpg']));
        imageinterlace($this->image['src'], in_array($format, $interlace) ? 1 : 0);

        ob_start();
        if ('jpg' == $format) {
            $quality = $this->getImageProperty(rex_managed_media::PROP_JPG_QUALITY, $addon->getConfig('jpg_quality', 85));
            imagejpeg($this->image['src'], null, $quality);
        } elseif ('png' == $format) {
            $compression = $this->getImageProperty(rex_managed_media::PROP_PNG_COMPRESSION, $addon->getConfig('png_compression', 5));
            ...
            $quality = $this->getImageProperty(rex_managed_media::PROP_WEBP_QUALITY, $addon->getConfig('webp_quality', 85));
