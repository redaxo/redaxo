...
""))&&c.href!==location.href+"#")c={url:c.href,container:b(c).attr("data-pjax"),target:c,fragment:null},e(b.extend({},c,f)),a.preventDefault()}function B(a,c,f){f=k(c,f);c=a.currentTarget;if("FORM"!==c.tagName.toUpperCase())throw"$.pjax.submit requires a form element";c={type:c.method.toUpperCase(),url:c.action,data:b(c).serializeArray(),container:b(c).attr("data-pjax"),target:c,fragment:null};e(b.extend({},c,f));a.preventDefault()}function e(a){function c(a,c){var d=b.Event(a,{relatedTarget:f});
g.trigger(d,c);return!d.isDefaultPrevented()}a=b.extend(!0,{},b.ajaxSettings,e.defaults,a);b.isFunction(a.url)&&(a.url=a.url());var f=a.target,d=r(a.url).hash,g=a.context=u(a.container);a.data||(a.data={});a.data._pjax=g.selector;var l;a.beforeSend=function(b,d){"GET"!==d.type&&(d.timeout=0);b.setRequestHeader("X-PJAX","true");b.setRequestHeader("X-PJAX-Container",g.selector);if(!c("pjax:beforeSend",[b,d]))return!1;0<d.timeout&&(l=setTimeout(function(){c("pjax:timeout",[b,a])&&b.abort("timeout")},
d.timeout),d.timeout=0);a.requestUrl=r(d.url).href};a.complete=function(b,d){l&&clearTimeout(l);c("pjax:complete",[b,d,a]);c("pjax:end",[b,a])};a.error=function(b,d,e){var f=v("",b,a);b=c("pjax:error",[b,d,e,a]);"GET"==a.type&&("abort"!==d&&b)&&s(f.url)};a.success=function(f,j,l){var h=v(f,l,a);if(h.contents){e.state={id:a.id||(new Date).getTime(),url:h.url,title:h.title,container:g.selector,fragment:a.fragment,timeout:a.timeout};(a.push||a.replace)&&window.history.replaceState(e.state,h.title,h.url);
h.title&&(document.title=h.title);g.html(h.contents);"number"===typeof a.scrollTo&&b(window).scrollTop(a.scrollTo);(a.replace||a.push)&&window._gaq&&_gaq.push(["_trackPageview"]);if(""!==d){var k=r(h.url);k.hash=d;e.state.url=k.href;window.history.replaceState(e.state,h.title,k.href);h=b(k.hash);h.length&&b(window).scrollTop(h.offset().top)}c("pjax:success",[f,j,l,a])}else s(h.url)};e.state||(e.state={id:(new Date).getTime(),url:window.location.href,title:document.title,container:g.selector,fragment:a.fragment,
timeout:a.timeout},window.history.replaceState(e.state,document.title));var j=e.xhr;j&&4>j.readyState&&(j.onreadystatechange=b.noop,j.abort());e.options=a;j=e.xhr=b.ajax(a);if(0<j.readyState){if(a.push&&!a.replace){var k=e.state.id,p=g.clone().contents();m[k]=p;for(n.push(k);q.length;)delete m[q.shift()];for(;n.length>e.defaults.maxCacheLength;)delete m[n.shift()];window.history.pushState(null,"",w(a.requestUrl))}c("pjax:start",[j,a]);c("pjax:send",[j,a])}return e.xhr}function C(a,c){return e(b.extend({url:window.location.href,
