...
$list->setColumnFormat('remove_session', 'custom', static function($params) {
    // prevent removing the current session
    if ($params['list']->getValue('session_id') === session_id()) {
        return '';
    }
    return $params['list']->getColumnLink('remove_session', $params['value']);
});
