    ...
    if (!is_file(Path::media($media->getFileName()))) {
        ...
        if (rex_media::isImageType(File::extension($media->getFileName()))) {
            ...
            if ('svg' != File::extension($media->getFileName())) {
                $thumbnail = '<img class="thumbnail" src="' . MediaManagerManager::getUrl('rex_media_small', urlencode($media->getFileName()), $media->getValue('updatedate')) . '" width="100" alt="' . $alt . '" title="' . $alt . '" loading="lazy" />';
            }
        }
    }
