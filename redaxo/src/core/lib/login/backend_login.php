...
class rex_backend_login extends rex_login
{
        ...
        if ($cookiekey = rex_cookie($cookiename, 'string')) {
            if (!$userId) {
                ...
                if ($sql->getRows() == 1) {
                    ...
                    rex_response::sendCookie($cookiename, $cookiekey, ['expires' => time() + 60 * 60 * 24 * 365, 'httponly' => true, 'samesite' => 'strict']);
                } else {
    ...
    {
        rex_response::sendCookie(self::getStayLoggedInCookieName(), '', ['expires' => time() - 3600]);
    }
