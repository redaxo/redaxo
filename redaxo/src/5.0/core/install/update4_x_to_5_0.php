...
if ($eventType == REX_A1_IMPORT_EVENT_PRE)
{
  ...
  $update->setQuery("ALTER TABLE `". $REX['TABLE_PREFIX'] ."media` CHANGE `revision` `revision` INT( 11 ) NOT NULL DEFAULT '0';");
  $update->setQuery("ALTER TABLE `". $REX['TABLE_PREFIX'] ."media_category` CHANGE `revision` `revision` INT( 11 ) NOT NULL DEFAULT '0';");
  ...
  $update->setQuery("ALTER TABLE ". $REX['TABLE_PREFIX'] ."media ADD INDEX `re_media_id` (`re_media_id`), ADD INDEX `category_id` (`category_id`);");
  $update->setQuery("ALTER TABLE ". $REX['TABLE_PREFIX'] ."media_category DROP PRIMARY KEY, ADD PRIMARY KEY (`id`), ADD INDEX `re_id` (`re_id`);");
