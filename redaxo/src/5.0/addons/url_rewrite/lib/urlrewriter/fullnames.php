...
class rex_urlRewriter_fullnames extends rex_urlRewriter
{
  ...
  public function prepare()
  {
    ...
    if(!rex_core::isBackend())
    {
      if(rex_request('article_id', 'int', 0) > 0)
  		{
        $this->setArticleId(rex_request('article_id', 'int', rex_core::getProperty('start_article_id')));
        return true;
  		}
      ...
      {
        $this->setArticleId(rex_core::getProperty('start_article_id'));
        return true;
      }
      ...
      }elseif($article_id == -1)
      {
				...
				if($article_id == -1)
	      {
					...
					if(rex_request('article_id', 'int', 0) > 0)
						...
						$article_id = rex_core::getProperty('notfound_article_id');
				}
      }
  ...
  public function generatePathnames($params)
  {
    ...
    if($where != '')
    {
      $db = rex_sql::factory();
      // $db->debugsql=true;
      $db->setQuery('SELECT id,clang,path,startpage FROM '. rex_core::getTablePrefix() .'article WHERE '. $where.' and revision=0');
