...
function rex_version_header($params)
{
	...
	$rex_version_article = rex_core::getProperty('login')->getSessionVar("rex_version_article");
	if(!is_array($rex_version_article))
		$rex_version_article = array();

	$working_version_empty = TRUE;
	$gw = rex_sql::factory();
	$gw->setQuery('select * from '.rex_core::getTablePrefix().'article_slice where article_id='.$params['article_id'].' and clang='.$params['clang'].' and revision=1 LIMIT 1');
	if($gw->getRows()>0)
	...
	switch($func)
	{
		case("copy_work_to_live"):
		  ...
		  }else if(!rex_core::getUser()->hasPerm('version[only_working_version]'))
		  {
  ...
  if(rex_core::getUser()->hasPerm('version[only_working_version]'))
  {
		$rex_version_article[$params['article_id']] = 1;
  	unset($revisions[0]);
  }

	rex_core::getProperty('login')->setSessionVar("rex_version_article", $rex_version_article);
