...
function rex_moveSlice($slice_id, $clang, $direction)
{
  ...
  $CM = rex_sql::factory();
  $CM->setQuery("select * from " . rex_core::getTablePrefix() . "article_slice where id='$slice_id' and clang=$clang");
  if ($CM->getRows() == 1)
  {
    // prepare sql for later saving
    $upd = rex_sql::factory();
    $upd->setTable(rex_core::getTablePrefix() . "article_slice");
    $upd->setWhere(array(
      'id' => $slice_id
    ));
...
function rex_deleteSlice($slice_id)
{
  ...
  $curr->setQuery('SELECT * FROM ' . rex_core::getTablePrefix() . 'article_slice WHERE id=' . $slice_id);
  if($curr->getRows() != 1)
  {
    return false;
  }
  ...
  // delete the slice
  $del = rex_sql::factory();
  $del->setQuery('DELETE FROM ' . rex_core::getTablePrefix() . 'article_slice WHERE id=' . $slice_id);
...
function rex_execPreViewAction($module_id, $function, $REX_ACTION)
{
  ...
  $ga = rex_sql::factory();
  $ga->setQuery('SELECT a.id, preview FROM '.rex_core::getTablePrefix().'module_action ma,'. rex_core::getTablePrefix(). 'action a WHERE preview != "" AND ma.action_id=a.id AND module_id='. $module_id .' AND ((a.previewmode & '. $modebit .') = '. $modebit .')');
...
function rex_execPreSaveAction($module_id, $function, $REX_ACTION)
{
  ...
  $ga = rex_sql::factory();
  $ga->setQuery('SELECT a.id, presave FROM ' . rex_core::getTablePrefix() . 'module_action ma,' . rex_core::getTablePrefix() . 'action a WHERE presave != "" AND ma.action_id=a.id AND module_id=' . $module_id . ' AND ((a.presavemode & ' . $modebit . ') = ' . $modebit . ')');
...
function rex_execPostSaveAction($module_id, $function, $REX_ACTION)
{
  ...
  $ga = rex_sql::factory();
  $ga->setQuery('SELECT a.id, postsave FROM ' . rex_core::getTablePrefix() . 'module_action ma,' . rex_core::getTablePrefix() . 'action a WHERE postsave != "" AND ma.action_id=a.id AND module_id=' . $module_id . ' AND ((a.postsavemode & ' . $modebit . ') = ' . $modebit . ')');
...
function rex_article2startpage($neu_id){
  ...
  $neu = rex_sql::factory();
  $neu->setQuery("select * from ".rex_core::getTablePrefix()."article where id=$neu_id and startpage=0 and clang=0");
  ...
  // alten startartikel
  $alt = rex_sql::factory();
  $alt->setQuery("select * from ".rex_core::getTablePrefix()."article where id=$neu_cat_id and startpage=1 and clang=0");
  ...
  {
    // alter startartikel
    $alt->setQuery("select * from ".rex_core::getTablePrefix()."article where id=$neu_cat_id and startpage=1 and clang=$clang");

    // neuer startartikel
    $neu->setQuery("select * from ".rex_core::getTablePrefix()."article where id=$neu_id and startpage=0 and clang=$clang");

    // alter startartikel updaten
    $alt2 = rex_sql::factory();
    $alt2->setTable(rex_core::getTablePrefix()."article");
    $alt2->setWhere("id=$alt_id and clang=". $clang);
    $alt2->setValue("re_id",$neu_id);

    // neuer startartikel updaten
    $neu2 = rex_sql::factory();
    $neu2->setTable(rex_core::getTablePrefix()."article");
  ...
  $ia = rex_sql::factory();
  $articles->setQuery("select * from ".rex_core::getTablePrefix()."article where path like '%|$alt_id|%'");
  for($i=0;$i<$articles->getRows();$i++)
  {
    $iid = $articles->getValue("id");
    $ipath = str_replace("|$alt_id|","|$neu_id|",$articles->getValue("path"));

    $ia->setTable(rex_core::getTablePrefix()."article");
    $ia->setWhere('id='.$iid);
  ...
  $users->setQuery('UPDATE '. rex_core::getTablePrefix() .'user SET rights = REPLACE(rights, "#csw['. $alt_id .']#", "#csw['. $neu_id .']#")');
...
function rex_article2category($art_id){
  ...
  {
    // artikel
    $sql->setQuery('select re_id, name from '.rex_core::getTablePrefix()."article where id=$art_id and startpage=0 and clang=$clang");

    if (!isset($re_id))
      $re_id = $sql->getValue('re_id');

    // artikel updaten
    $sql->setTable(rex_core::getTablePrefix()."article");
...
function rex_category2article($art_id){
  ...
  $sql->setQuery('SELECT pid FROM '. rex_core::getTablePrefix() .'article WHERE re_id='. $art_id .' LIMIT 1');
  if ($sql->getRows() != 0)
    return false;
  ...
  {
    // artikel
    $sql->setQuery('select re_id, name from '.rex_core::getTablePrefix()."article where id=$art_id and startpage=1 and clang=$clang");
...
function rex_copyMeta($from_id, $to_id, $from_clang = 0, $to_clang = 0, $params = array ())
{
  ...
  $gc = rex_sql::factory();
  $gc->setQuery("select * from ".rex_core::getTablePrefix()."article where clang='$from_clang' and id='$from_id'");

  if ($gc->getRows() == 1)
  {
    $uc = rex_sql::factory();
    // $uc->debugsql = 1;
    $uc->setTable(rex_core::getTablePrefix()."article");
...
function rex_copyContent($from_id, $to_id, $from_clang = 0, $to_clang = 0, $from_re_sliceid = 0, $revision = 0)
{
  ...
  $gc = rex_sql::factory();
  $gc->setQuery("select * from ".rex_core::getTablePrefix()."article_slice where article_id='$from_id' and clang='$from_clang' and revision='$revision'");

  if ($gc->getRows() > 0)
  {
    $ins = rex_sql::factory();
    $ins->setTable(rex_core::getTablePrefix()."article_slice");
    $ctypes = array();

    $cols = rex_sql::factory();
    // $cols->debugsql = 1;
    $cols->setquery("SHOW COLUMNS FROM ".rex_core::getTablePrefix()."article_slice");
    foreach($gc as $slice)
    {
      foreach($cols as $col)
      {
...
function rex_copyArticle($id, $to_cat_id)
{
  ...
  foreach ($REX['CLANG'] as $clang => $clang_name)
  {
    ...
    $qry = 'select * from '.rex_core::getTablePrefix().'article where clang="'.$clang.'" and id="'. $id .'"';
    ...
    if ($from_sql->getRows() == 1)
    {
      ...
      $to_sql->setQuery('select * from '.rex_core::getTablePrefix().'article where clang="'.$clang.'" and startpage=1 and id="'. $to_cat_id .'"');
      ...
      if ($to_sql->getRows() == 1 || $to_cat_id == 0)
      {
        ...
        $art_sql->setTable(rex_core::getTablePrefix().'article');
        ...
        $revisions->setQuery("select revision from ".rex_core::getTablePrefix()."article_slice where prior=1 AND ctype=1 AND article_id='$id' AND clang='$clang'");
...
function rex_moveArticle($id, $from_cat_id, $to_cat_id)
{
  ...
  foreach ($REX['CLANG'] as $clang => $clang_name)
  {
    // validierung der id & from_cat_id
    $from_sql = rex_sql::factory();
    $from_sql->setQuery('select * from '.rex_core::getTablePrefix().'article where clang="'. $clang .'" and startpage<>1 and id="'. $id .'" and re_id="'. $from_cat_id .'"');
...
function rex_moveCategory($from_cat, $to_cat)
{
  ...
  else
  {
    ...
    $fcat->setQuery("select * from ".rex_core::getTablePrefix()."article where startpage=1 and id=$from_cat and clang=0");

    $tcat = rex_sql::factory();
    $tcat->setQuery("select * from ".rex_core::getTablePrefix()."article where startpage=1 and id=$to_cat and clang=0");
    ...
    else
    {
      ...
      // $gcats->debugsql = 1;
      $gcats->setQuery("select * from ".rex_core::getTablePrefix()."article where path like '".$from_path."%' and clang=0");
      ...
      for($i=0;$i<$gcats->getRows();$i++)
      {
        ...
        $up->setTable(rex_core::getTablePrefix()."article");
        $up->setWhere("id=$icid");
      ...
      foreach($REX['CLANG'] as $clang => $clang_name)
      {
        $gmax->setQuery("select max(catprior) from ".rex_core::getTablePrefix()."article where re_id=$to_cat and clang=".$clang);
