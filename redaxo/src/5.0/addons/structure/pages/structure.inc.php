...
$clang       = rex_request('clang',       'rex-clang-id', rex_core::getProperty('start_article_id'));
...
// --------------------------------------------- Rechte prŸfen
$KATPERM = rex_core::getUser()->hasCategoryPerm($category_id);
...
$add_category = '';
if ($KATPERM && !rex_core::getUser()->hasPerm('editContentOnly[]'))
{
...
if(count($mountpoints)>0 && $category_id == 0)
{
  ...
  $KAT->setQuery('SELECT COUNT(*) as rowCount FROM '.rex_core::getTablePrefix().'article WHERE id IN ('.$re_id.') AND startpage=1 AND clang='. $clang .' ORDER BY catname');
}else
{
  $KAT->setQuery('SELECT COUNT(*) as rowCount FROM '.rex_core::getTablePrefix().'article WHERE re_id='. $category_id .' AND startpage=1 AND clang='. $clang .' ORDER BY catprior');
}
...
{
  $re_id = implode(',', $mountpoints);
  $KAT->setQuery('SELECT * FROM '.rex_core::getTablePrefix().'article WHERE id IN ('.$re_id.') AND startpage=1 AND clang='. $clang .' ORDER BY catname LIMIT '. $catPager->getCursor(). ','. $catPager->getRowsPerPage());
}else
{
  $KAT->setQuery('SELECT * FROM '.rex_core::getTablePrefix().'article WHERE re_id='. $category_id .' AND startpage=1 AND clang='. $clang .' ORDER BY catprior LIMIT '. $catPager->getCursor(). ','. $catPager->getRowsPerPage());
}
...
if ($function == 'add_cat' && $KATPERM && !rex_core::getUser()->hasPerm('editContentOnly[]'))
{
  $add_td = '';
  if (rex_core::getUser()->hasPerm('advancedMode[]'))
  {
...
for ($i = 0; $i < $KAT->getRows(); $i++)
{
  ...
  if ($KATPERM)
  {
    if (rex_core::getUser()->isAdmin() || $KATPERM && rex_core::getUser()->hasPerm('publishCategory[]'))
    {
    ...
    else
    {
      ...
      if (!rex_core::getUser()->hasPerm('editContentOnly[]'))
      {
  ...
  elseif (/*rex_core::getUser()->hasPerm('csr['. $i_category_id .']') ||*/ rex_core::getUser()->hasPerm('csw['. $i_category_id .']'))
  {
      // --------------------- KATEGORIE WITH READ
      $add_td = '';
      if (rex_core::getUser()->hasPerm('advancedMode[]'))
...
if ($category_id > 0 || ($category_id == 0 && !rex_core::getUser()->hasMountpoints()))
{
  ...
  // --------------------- ARTIKEL ADD FORM
  if ($function == 'add_art' && $KATPERM && !rex_core::getUser()->hasPerm('editContentOnly[]'))
  {
    $defaultTemplateId = rex_core::getProperty('default_template_id');
    ...
    }else
    {
      ...
      $sql2->setQuery('SELECT template_id FROM '.rex_core::getTablePrefix().'article WHERE id='. $category_id .' AND clang='. $clang .' AND startpage=1');
      if ($sql2->getRows() == 1)
  ...
  for ($i = 0; $i < $sql->getRows(); $i++)
  {
    ...
    }elseif ($KATPERM)
    {
      ...
      {
        if (rex_core::getUser()->isAdmin() || $KATPERM && rex_core::getUser()->hasPerm('publishArticle[]'))
