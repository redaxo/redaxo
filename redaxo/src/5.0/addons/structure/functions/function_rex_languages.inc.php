...
// admin[]
// clang[xx], clang[0]
// rex_core::getUser()->hasPerm("csw[0]")
...
if ($num_clang>1)
{
  ...
  foreach($REX['CLANG'] as $key => $val)
  {
     ...
     $lang['url'] = '';
     if (!rex_core::getUser()->isAdmin() && !rex_core::getUser()->hasPerm('clang[all]') && !rex_core::getUser()->hasPerm('clang['. $key .']'))
     {
       if ($clang == $key)
       {
         $stop = true;
     ...
     else
     {
       ...
       $lang['url'] = 'index.php?page='. rex_core::getProperty('page') .'&amp;clang='. $key . $sprachen_add .'&amp;ctype='. $ctype;
     }
     $i++;
     $languages[] = $lang;
  }
