...
$KAT = rex_sql::factory();
// $KAT->debugsql = true;
$KAT->setQuery("SELECT * FROM ".rex_core::getTablePrefix()."article WHERE id=$category_id AND startpage=1 AND clang=$clang");

if ($KAT->getRows()!=1 || !rex_core::getUser()->hasCategoryPerm($category_id))
{
...
else
{
  ...
  {
    $catid = $parent->getId();
    if (rex_core::getUser()->hasCategoryPerm($catid))
    {
