...
{
	$content = "";
	$file = $REX['SRC_PATH'] ."/addons/community/plugins/setup/config.inc.php";
	$i=0;
...
if ($func == "module" && $request_module != "")
{
	...
	foreach($modules as $module)
	{
		if ($request_module == $module[0].".".$module[1])
		{
			...
			$in = rex_get_file_contents($REX['SRC_PATH'] ."/addons/community/plugins/".$module[0]."/modules/".$module[1]."_in.module");
			$out = rex_get_file_contents($REX['SRC_PATH'] ."/addons/community/plugins/".$module[0]."/modules/".$module[1]."_out.module");
...
if ($func == "template" && $request_template != "")
{
	...
	foreach($templates as $template)
	{
		if ($request_template == $template[0].".".$template[1])
		{
			...
			$content = rex_get_file_contents($REX['SRC_PATH'] ."/addons/community/plugins/".$template[0]."/templates/".$template[1].".template");
...
if ($func == "email" && $request_email != "")
{
	...
	foreach($emails as $email)
	{
		if ($request_email == $email[0].".".$email[1])
		{
			...
			$body = rex_get_file_contents($REX['SRC_PATH'] ."/addons/community/plugins/".$email[0]."/emails/".$email[1].".email");
