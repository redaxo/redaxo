...
// ---- validate type_id
$sql = rex_sql::factory();
$sql->setQuery('SELECT * FROM '. rex_core::getTablePrefix().'679_types WHERE id='. $type_id);
if($sql->getRows() != 1)
{
  unset($type_id);
}
...
//  $info = rex_i18n::msg('imanager_cache_files_removed', $counter);
}
...
if($func == 'delete' && $effect_id > 0)
{
  $sql = rex_sql::factory();
//  $sql->debugsql = true;
  $sql->setTable(rex_core::getTablePrefix().'679_type_effects');
  $sql->setWhere('id='. $effect_id . ' LIMIT 1');

  if($sql->delete())
  {
     $info = rex_i18n::msg('imanager_effect_deleted') ;
  }
  else
  {
...
if ($func == '' && $type_id > 0)
{
  echo rex_content_block(rex_i18n::msg('imanager_effect_list_header', htmlspecialchars($typeName)));

  $query = 'SELECT * FROM '.rex_core::getTablePrefix().'679_type_effects WHERE type_id='.$type_id .' ORDER BY prior';

	$list = rex_list::factory($query);
  $list->setNoRowsMessage(htmlspecialchars(rex_i18n::msg('imanager_effect_no_effects')));
  $list->setCaption(htmlspecialchars(rex_i18n::msg('imanager_effect_caption', $typeName)));
  $list->addTableAttribute('summary', htmlspecialchars(rex_i18n::msg('imanager_effect_summary', $typeName)));
  $list->addTableColumnGroup(array(40, '*', 40, 130, 130));
	...
	$list->setColumnLabel('effect',htmlspecialchars(rex_i18n::msg('imanager_type_name')));
	$list->setColumnLabel('prior',htmlspecialchars(rex_i18n::msg('imanager_type_prior')));

	// icon column
  $thIcon = '<a class="rex-i-element rex-i-generic-add" href="'. $list->getUrl(array('type_id' => $type_id, 'func' => 'add')) .'"><span class="rex-i-element-text">'. htmlspecialchars(rex_i18n::msg('imanager_effect_create')) .'</span></a>';
  ...
  $funcs = rex_i18n::msg('imanager_effect_functions');
  $list->addColumn($funcs, rex_i18n::msg('imanager_effect_edit'), -1, array('<th colspan="2">###VALUE###</th>','<td>###VALUE###</td>'));
  $list->setColumnParams($funcs, array('func' => 'edit', 'type_id' => $type_id, 'effect_id' => '###id###'));

  $delete = 'deleteCol';
  $list->addColumn($delete, rex_i18n::msg('imanager_effect_delete'), -1, array('','<td>###VALUE###</td>'));
        ...
        $func == 'edit' && $effect_id > 0 && $type_id > 0)
{
  ...
  if($func == 'edit')
  {
    $formLabel = rex_i18n::msg('imanager_effect_edit_header', htmlspecialchars($typeName));
  }
  else if ($func == 'add')
  {
    $formLabel = rex_i18n::msg('imanager_effect_create_header', htmlspecialchars($typeName));
  }

	$form = rex_form::factory(rex_core::getTablePrefix().'679_type_effects',$formLabel,'id='.$effect_id);
  ...
  $form->addHiddenField('type_id', $type_id);
	...
	$field->setLabel(rex_i18n::msg('imanager_effect_name'));
  ...
  $field->setLabel(rex_i18n::msg('imanager_effect_prior'));
