...
class rex_thumbnail
{
  ...
  /**
   * Ausschnitt aus dem Bild auf bestimmte gr��e zuschneiden
   *
  ...
  function generateImage($file = null, $show = true)
  {
	...
	function checkCacheFiles()
	{
		...
		if ($REX['ADDON']['image_resize']['max_cachefiles']<=count($glo))
		{
			...
			// nur das �lteste Cachefile l�schen
			foreach($glo as $gl)
			{
  ...
  function deleteCache($filename = '')
  {
	  ...
	  $folders = array();
    $folders[] = $REX['SRC_PATH'] .'/generated/files/';
  ...
  function createFromUrl($rex_resize)
  {
    ...
    $offset = $resize[6];
	  ...
	  $cachepath = $REX['SRC_PATH'] .'/generated/files/image_resize__'.$filters.$rex_resize;
	  ...
	  $thumb->img_filename = $imagefile;
  	$thumb->img_cachepath = $REX['SRC_PATH'] .'/generated/files/';
